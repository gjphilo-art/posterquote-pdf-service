...
function normalizePayload(body) {
  ...
  normalized.order_id = toPositiveInt(firstDefined(
    body?.order_id,
    body?.orderId,
    body?.order,
    body?.meta?.order_id,
    body?.meta?.orderId,
    getNested(body, 'context.order_id'),
    getNested(body, 'context.orderId')
  ));
  normalized.item_id = toPositiveInt(firstDefined(
    body?.item_id,
    body?.itemId,
    body?.order_item_id,
    body?.item,
    body?.meta?.item_id,
    body?.meta?.itemId,
    getNested(body, 'context.item_id'),
    getNested(body, 'context.itemId')
  ));
  normalized.product_id = toPositiveInt(firstDefined(body?.product_id, body?.productId, body?.product));
  normalized.product_name = String(firstDefined(body?.product_name, body?.productName, body?.name, '') ?? '');
  normalized.filename = String(firstDefined(body?.filename, body?.file_name, body?.fileName, '') ?? '');
  normalized.svg = String(firstDefined(body?.svg, body?.final_svg, body?.finalSvg, body?.svg_markup, body?.svgMarkup, '') ?? '');
  normalized.svg_url = String(firstDefined(body?.svg_url, body?.svgUrl, '') ?? '');
  normalized.callback_url = String(firstDefined(body?.callback_url, body?.callbackUrl, body?.callback, '') ?? '');
  normalized.callback_token = String(firstDefined(body?.callback_token, body?.callbackToken, body?.token, '') ?? '');
  normalized.source_site = String(firstDefined(body?.source_site, body?.sourceSite, body?.site, '') ?? '');
  return normalized;
}
...
async function postCallback(job, extra = {}) {
  ...
  if (!job.order_id || !job.item_id) {
    log('[CALLBACK]', job.job_id, 'warning: callback ids missing', `order=${job.order_id || 0}`, `item=${job.item_id || 0}`);
  }
  const payload = {
...
app.post('/convert', requireAuth, async (req, res) => {
  ...
  log('[CONVERT]', 'body keys', Object.keys(req.body || {}).join(',') || '(none)');
  log('[CONVERT]', 'normalized ids', `order=${payload.order_id || 0}`, `item=${payload.item_id || 0}`, `product=${payload.product_id || 0}`);
